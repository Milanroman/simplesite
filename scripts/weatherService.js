function getWeatherData (lang, fnOK, fnError) {
    ...
    function locSuccess(position) {
        ...
        // If the cache is newer than 5 minutes, use the cache
        if (cache && cache.timestamp && cache.timestamp > currDate.getTime() - 5*60*1000){
            fnOK.call(this, cache.data);
        } else {
            $.getJSON(
                'http://api.openweathermap.org/data/2.5/forecast?lat=' + position.coords.latitude + '&lon=' +
                position.coords.longitude + '&cnt=8&units=metric&APPID=19e6212ba1a15130d764c8ce642a543b' + '&lang=' + lang + '&callback=?',
                function (response) {
                    // Store the cache
